...
def _extract_search_outputs(
    ...
    action_index = int(single.action_index)
    return weights, float(single.root_value), candidates, best, action_index
...
def _pad_trajectory_steps(
    ...
    *,
    ...
    policy_masks = np.zeros((window,), dtype=np.float32)
    ...
    for step_index, candidates in enumerate(step_candidates):
        ...
        policy_masks[step_index] = step_policy_masks[step_index]

    return padded_policies, search_values, padded_candidates, padded_best, policy_masks
...
def reanalyze_policy_batch(
    ...
    *,
    ...
    Returns ``policy_targets``, ``search_values``, ``policy_candidates``,
    ``best_actions``, and ``policy_masks`` with shape ``[B, K+1, ...]``.
    """
    ...
    step_policy_masks: dict[tuple[int, int], float] = {}
    ...
    for chunk_start in range(0, total_progress, search_batch_size):
        ...
        for lane, (batch_index, step_index) in enumerate(chunk_items):
            weights, value, candidates, best, action_index = _extract_search_outputs(result, lane)
            ...
            step_best[key] = best
            policy_mask = 1.0
            if weights.size > 0 and int(np.argmax(weights)) != action_index:
                policy_mask = 0.0
            step_policy_masks[key] = policy_mask
    ...
    policy_masks = np.ones((batch_size, window), dtype=np.float32)
    ...
    for batch_index in range(count):
        ...
        traj_masks = [step_policy_masks[(batch_index, step_index)] for step_index in range(window)]
        padded_policies, padded_values, padded_candidates, padded_best, padded_masks = _pad_trajectory_steps(
            traj_candidates,
            traj_policies,
            traj_best,
            traj_values,
        ...
        policy_masks[batch_index] = padded_masks
    ...
    return policy_targets, search_values, stacked, best_actions, policy_masks
...
def reanalyze_training_batch(
    ...
    *,
    ...
    """Backward-compatible wrapper returning policy tensors without masks."""
    policy_targets, search_values, policy_candidates, best_actions, _ = reanalyze_policy_batch(
        planner,
        observations,
    ...
    return policy_targets, search_values, policy_candidates, best_actions
